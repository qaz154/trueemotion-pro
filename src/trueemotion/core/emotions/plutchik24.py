...
EMOTION_KEYWORDS: Dict[str, Tuple[str, ...]] = {
    # 喜悦
    "joy": (
        "开心", "高兴", "快乐", "愉快", "喜悦", "欢快", "幸福", "美好", "棒", "爽", "耶", "哈哈", "嘻嘻", "乐", "欢欣", "雀跃", 
        "得意", "满足", "欣慰", "舒畅", "痛快", "美滋滋", "乐呵呵", "笑"
    ),
    "ecstasy": (
        "狂喜", "兴奋", "疯了", "太开心了", "爽翻了", "嗨", "起飞", "爆炸", "激动", "热血", "沸腾", "癫狂", "欢天喜地", "心花怒放", 
        "手舞足蹈", "欣喜若狂", "乐不可支", "欢欣雀跃", "亢奋", "嗨起来", "爽呆", "爆炸开心"
    ),
    "serenity": (
        "宁静", "平静", "安宁", "安详", "恬静", "祥和", "从容", "淡然", "心如止水", "波澜不惊", "静谧", "悠然", "闲适", "恬淡", "平和"
    ),

    # 悲伤
    "sadness": (
        "难过", "伤心", "悲伤", "悲痛", "失落", "沮丧", "郁闷", "压抑", "绝望", "痛苦", "哀伤", "凄凉", "沉重", "消沉", "低落", 
        "忧郁", "伤感", "想哭", "泪目", "心碎", "哀愁", "凄惨", "苦涩", "酸楚", "酸涩"
    ),
    "grief": (
        "痛哭", "崩溃", "心碎", "悲痛欲绝", "泪流满面", "嚎啕", "泣不成声", "捶胸顿足", "痛不欲生", "万念俱灰", "心如刀割", "肝肠寸断", 
        "撕心裂肺", "痛彻心扉", "悲恸", "哀恸", "绝哀"
    ),
    "melancholy": (
        "忧郁", "郁闷", "惆怅", "落寞", "寂寥", "萧索", "凄清", "苍凉", "感伤", "惘然", "惆怅若失", "百感交集", "思绪万千"
    ),
    "loneliness": (
        "孤独", "寂寞", "孤单", "空落落", "冷清", "孤零零", "寂寥", "落寞", "空虚", "茫然", "无所依托", "无所事事", "心里空空的"
    ),
    "pensiveness": (
        "沉思", "深思", "思索", "思量", "沉吟", "若有所思", "冥想", "思绪", "琢磨", "思考", "想一想", "陷入沉思", "凝思"
    ),

    # 愤怒
    "anger": (
        "生气", "愤怒", "气愤", "恼火", "发火", "大火", "暴怒", "气", "怒", "不爽", "讨厌", "可恶", "恨", "气死", "抓狂", 
        "崩溃", "火大", "怒了", "发怒", "大发雷霆", "怒不可遏", "恼怒", "愤慨", "怨恨", "憎恶"
    ),
    "rage": (
        "暴怒", "狂怒", "怒火中烧", "火冒三丈", "大发雷霆", "怒发冲冠", "暴跳如雷", "怒狠狠", "气炸", "气疯了", "恨之入骨", "怒不可遏", 
        "满腔怒火", "怒形于色"
    ),
    "annoyance": (
        "烦恼", "烦", "恼", "气恼", "烦躁", "心烦", "腻烦", "厌烦", "不耐烦", "不爽", "窝火", "憋屈", "郁闷", "烦人", "惹人烦"
    ),
    "contempt": (
        "鄙视", "蔑视", "藐视", "轻蔑", "看不起", "不屑", "鄙夷", "轻视", "小看", "瞧不起", "不屑一顾", "嗤之以鼻", "轻慢", "侮蔑", 
        "蔑弃"
    ),

    # 恐惧
    "fear": (
        "害怕", "恐惧", "担心", "担忧", "紧张", "怕", "畏", "惶恐", "不安", "惊恐", "惊吓", "胆怯", "心虚", "后怕", "发毛", 
        "哆嗦", "发抖", "惊悚"
    ),
    "terror": (
        "极度恐惧", "吓死", "吓坏", "恐怖", "可怕", "毛骨悚然", "胆战心惊", "心惊肉跳", "魂飞魄散", "惊骇", "惶恐", "震恐", "惧怕", 
        "畏怯", "颤抖"
    ),
    "anxiety": (
        "焦虑", "忧虑", "着急", "担忧", "担心", "不安", "紧张", "烦躁", "坐立不安", "忐忑", "心神不宁", "慌张", "慌忙", "焦急", 
        "牵挂", "顾虑"
    ),
    "apprehension": (
        "忧虑", "忧心", "隐隐不安", "有所顾虑", "惴惴不安", "提心吊胆", "心存疑虑", "担心", "不安", "忐忑", "忧惧", "惴惴", "顾虑重重"
    ),

    # 厌恶
    "disgust": (
        "恶心", "厌恶", "讨厌", "反感", "腻", "烦", "腻歪", "嫌弃", "憎恶", "作呕", "反胃", "鄙夷", "不屑", "反感的", "吐了", 
        "油腻", "土"
    ),
    "loathing": (
        "深恶痛绝", "极其厌恶", "作呕", "恶心至极", "厌恶至极", "唾弃", "不齿", "鄙夷", "蔑视", "看不起", "藐视", "轻视", "小看", 
        "腻味", "恶心透顶"
    ),
    "boredom": (
        "无聊", "厌倦", "乏味", "腻", "烦", "没意思", "倦怠", "疲惫", "疲倦", "累", "没劲", "无趣", "单调", "枯燥", "腻烦", 
        "兴味索然", "百无聊赖"
    ),

    # 惊讶
    "surprise": (
        "惊讶", "吃惊", "意外", "震惊", "惊", "哇", "呀", "咦", "诶", "竟然", "居然", "万万没想到", "没想到", "惊呆", "吓", 
        "吓到", "惊喜", "惊吓"
    ),
    "astonishment": (
        "惊骇", "震惊", "惊呆", "目瞪口呆", "瞠目结舌", "大惊", "惊呼", "惊叹", "惊异", "惊诧", "诧异", "匪夷所思", "难以置信", 
        "不可思议", "惊愕", "惊倒"
    ),
    "distraction": (
        "分心", "走神", "心不在焉", "神游", "恍惚", "发呆", "出神", "注意力不集中", "心猿意马", "魂不守舍", "漫不经心"
    ),

    # 信任
    "trust": (
        "相信", "信任", "依赖", "放心", "可靠", "托付", "信仰", "信念", "认可", "肯定", "确定", "踏实", "安心", "信赖", "依靠", 
        "交给", "信心", "安全感"
    ),
    "admiration": (
        "钦佩", "敬佩", "佩服", "崇拜", "仰慕", "羡慕", "欣赏", "赞叹", "赞许", "认可", "赞同", "称颂", "颂扬", "赞美", "表扬", 
        "称赞", "赞赏", "敬仰"
    ),
    "acceptance": (
        "接受", "认可", "认同", "同意", "赞成", "肯定", "承认", "容纳", "包容", "理解", "体谅", "谅解", "宽恕", "原谅", "赞许", 
        "首肯"
    ),
    "interest": (
        "兴趣", "好奇", "感兴趣", "关注", "注意", "留意", "探究", "想了解", "怎么回事", "有意思", "有看头", "吸引", "好奇心想"
    ),

    # 期待
    "anticipation": (
        "期待", "希望", "盼望", "憧憬", "等待", "期望", "想", "将要", "即将", "马上", "快要", "就要", "指望", "愿望", "向往"
    ),
    "hope": (
        "希望", "盼望", "期待", "憧憬", "向往", "期望", "指望", "愿望", "光明", "曙光", "信心", "相信会好", "会好起来的"
    ),
    "optimism": (
        "乐观", "积极", "阳光", "向上", "正面", "充满希望", "有信心", "加油", "前途光明", "积极向上", "乐观其成", "抱持希望"
    ),
    "vigilance": (
        "警觉", "警惕", "戒备", "提防", "谨慎", "小心", "注意", "留意", "关注", "盯住", "监视", "守卫", "警戒", "敏锐", "机警"
    ),
    "curiosity": (
        "好奇", "好奇心", "想知道", "想了解", "怎么回事", "为什么", "什么情况", "有意思", "有趣", "探索", "钻研", "研究", "探讨"
    ),

    # 复杂情感
    "love": (
        "爱", "喜欢", "爱慕", "喜爱", "爱上", "心爱", "亲爱的", "甜蜜", "温馨", "浪漫", "表白", "追求", "想念", "思念", "牵挂", 
        "在乎", "幸福"
    ),
    "gratitude": (
        "感谢", "谢谢", "感激", "感恩", "多谢", "致谢", "谢意", "感激不尽", "没齿难忘", "铭感于心", "感谢不尽", "致以谢意"
    ),
    "guilt": (
        "内疚", "愧疚", "自责", "抱歉", "对不起", "后悔", "懊悔", "悔恨", "惭愧", "心虚", "过意不去", "不安", "歉意", "赔罪", 
        "请罪", "悔过", "反省"
    ),
    "shame": (
        "羞耻", "丢脸", "不好意思", "惭愧", "丢人", "脸红", "汗颜", "无地自容", "羞愧", "羞耻感", "面子", "丢人现眼"
    ),
    "regret": (
        "后悔", "遗憾", "惋惜", "可惜", "懊悔", "悔不当初", "悔过", "追悔", "扼腕", "叹息"
    ),
    "envy": (
        "嫉妒", "羡慕", "眼红", "眼馋", "眼热", "醋意", "酸", "吃醋", "不忿", "不服气", "凭什么", "攀比", "羡慕嫉妒"
    ),
    "pride": (
        "自豪", "骄傲", "得意", "成就感", "自信", "自豪感", "得意洋洋", "飘飘然", "自满", "自负", "傲娇", "自尊"
    ),
    "despair": (
        "绝望", "崩溃", "无望", "失望", "无助", "无奈", "无能为力", "放弃", "自暴自弃", "没希望", "没救了", "完了", "万念俱灰", "心死", 
        "消沉", "没有希望", "希望破灭", "人生没有希望", "前途黑暗", "看不到希望", "活着没意思", "生无可恋", "一切都没意义"
    ),
    "relief": (
        "如释重负", "松了一口气", "轻松", "放心", "松了口气", "卸下重担", "轻松愉快", "放下心头大石", "心安"
    ),
    "contentment": (
        "满足", "满意", "知足", "惬意", "舒服", "舒适", "美滋滋", "心满意足", "称心如意", "如愿以偿", "满足感", "幸福感"
    ),
    "amusement": (
        "好笑", "有趣", "滑稽", "幽默", "逗", "可乐", "可笑", "搞笑", "逗笑", "趣味", "笑死我了", "太逗了", "笑死", "哈哈笑"
    ),
    "confusion": (
        "困惑", "疑惑", "不解", "迷糊", "茫然", "晕", "懵", "搞不懂", "糊涂", "不明白", "不清楚", "疑问", "困惑不解", "一头雾水"
    ),
    "embarrassment": (
        "尴尬", "不好意思", "难为情", "脸红", "羞涩", "腼腆", "害羞", "窘迫", "尴尬症", "脚趾抠地", "脸红心跳"
    ),
    "compassion": (
        "同情", "心疼", "可怜", "体恤", "怜悯", "心软", "不忍", "怜惜", "心有戚戚", "感同身受", "不忍心", "于心不忍"
    ),
    "longing": (
        "想念", "思念", "渴望", "惦记", "牵挂", "眷恋", "怀念", "向往", "期盼", "念想", "思念之情"
    ),
    "nostalgia": (
        "怀旧", "怀念", "回忆", "想念过去", "当年", "时光", "岁月", "追忆", "旧时", "往日", "忆当年", "怀念从前", "时光流逝"
    ),
    "sorrow": (
        "哀愁", "忧愁", "忧虑", "愁闷", "愁苦", "哀伤", "忧思", "郁闷", "愁绪", "忧伤", "哀怨", "悲戚", "凄然", "惆怅"
    ),

    # 复合情感
    "gratitude_love": (
        "感恩", "感激的爱", "温馨", "感动", "暖心", "窝心", "暖洋洋", "心暖", "感激不尽", "铭感于心", "感恩戴德"
    ),
    "hope_fear": (
        "忐忑", "惶恐", "焦虑", "担忧", "忧心忡忡", "心悬", "不安", "放心不下", "战战跟鞋", "如履薄冰", "惴惴不安"
    ),
    # 新增复合情感
    "frustration_hopelessness": (
        "无奈", "无助", "绝望", "无能为力", "算了", "就这样吧", "无所谓", "没意思", "无所谓了", "爱咋咋", "躺平", "摆烂"
    ),
    "love_admiration": (
        "崇拜", "仰慕", "钦佩", "心动", "暗恋", "crush", "喜欢", "欣赏", "被圈粉", "粉了", "路转粉", "始于颜值", "陷于才华"
    ),
    "painful_joy": (
        "喜极而泣", "喜极而悲", "激动", "热泪盈眶", "感动落泪", "泣不成声", "泪流满面", "哭了", "太激动了", "太感动了"
    ),
    "angry_fear": (
        "恐慌", "惊惧", "惧怕", "畏惧", "害怕", "惊恐", "惶恐不安", "心惊胆战", "坐立不安", "六神无主"
    ),
    "sadness_guilt": (
        "自责", "愧疚", "对不起", "过意不去", "心里不安", "亏欠", "对不起大家", "让大家失望了"
    ),
    "anger_sadness": (
        "怒其不争", "又气又难过", "气恼悲伤", "又怒又悲", "气得想哭", "又恨又心疼", "愤怒又伤心", "又气又心酸"
    ),
    "jealous_love": (
        "吃醋", "醋意", "酸了", "嫉妒", "占有欲", "吃醋了", "酸溜溜", "凭什么", "不公", "委屈"
    ),
    "shock_denial": (
        "不敢相信", "不可能", "不会吧", "假的吧", "开什么玩笑", "说笑吧", "骗人的吧", "我不信"
    ),
    "relief_sadness": (
        "如释重负又难过", "松口气又担心", "放心了但也失落", "终于没事了可是", "好了但感觉空落落的"
    ),
    "happy_sadness": (
        "又开心又难过", "悲喜交加", "五味杂陈", "复杂", "说不清", "开心也难过", "高兴也难受", "又笑又哭"
    ),
}
...
EMOTION_TRANSITIONS: Dict[str, List[str]] = {
    ...
    "anger": ["rage", "contempt", "annoyance", "frustration_hopelessness"],
    "fear": ["terror", "anxiety", "apprehension", "vigilance"],
